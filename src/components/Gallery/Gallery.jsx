...
export default function Gallery() {
  ...
  const updateVisibleItems = () => {
    ...
    for (let row = startRow; row <= endRow; row++) {
      for (let col = startCol; col <= endCol; col++) {
        ...
        const src = imageUrls[(itemNum - 1) % imageUrls.length];
        img.src = src;
        ...
        img.decoding = "async";
        img.onerror = () => {
          // Fallback to a local asset if remote image fails
          img.onerror = null;
          img.src = "/contact/contact-img.jpg";
        };
        item.appendChild(img);
        item.dataset.itemNum = String(itemNum);
  ...
  const expandItem = (item) => {
    ...
    const imgNum = parseInt(item.dataset.itemNum || "1");
